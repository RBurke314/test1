...
def Tracking():  
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        if facecount == 1:
            ...
            #cv2.imwrite("cap2.png", sub_face)
            #resize_image = cv2.resize(sub_face,((FACE_WIDTH), (FACE_HEIGHT)),interpolation=cv2.INTER_LANCZOS4)
            cv2.imwrite("subject66.stranger.png", sub_face)
            #cv2.imwrite("cap3.png", resize_image)
            # Display the resulting frame
            capture=cv2.imread("subject66.stranger.png")
...
while(True):
    input = raw_input("Would you capture an  (y/n) ").lower()
